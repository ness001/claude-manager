...
- [x] QuickActions section label "Quick Actions" rendered as a `<div>` (`src/components/dashboard/QuickActions.tsx:32-34`) — same heading-semantics defect. WCAG 1.3.1 / 2.4.6. Mirrored PR #61: switched to `<h3>`. — PR #64
...
- [x] `SessionListPanel` "+ New Session" header button (`src/components/sessions/SessionListPanel.tsx:189-199`) had a decorative `Plus` lucide icon next to the visible "New Session" label without `aria-hidden`, so screen readers announced "Plus, New Session" — redundant noise. WCAG 4.1.2 (Name, Role, Value). Mirrored PR #58 (SkillCard) and PR #68 (SkillsListView Create Skill): added `aria-hidden="true"` to the icon. — PR #70
...
- [x] `McpServerDetail` env-row reveal toggle (`src/components/mcp/McpServerDetail.tsx:108-135`) carried a generic `aria-label="Reveal value"`/`"Hide value"` — when an env block lists multiple secrets, SR users had no way to tell the toggles apart. Threaded the env-var key through `MaskedValue` as a `name` prop so the label becomes `"Reveal <KEY>"` / `"Hide <KEY>"`. WCAG 4.1.2 (Name, Role, Value). — PR #74
- [ ] _further investigation pending_
