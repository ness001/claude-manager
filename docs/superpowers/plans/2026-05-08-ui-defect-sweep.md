...
- [x] `McpServerForm` `KeyValueEditor` per-row remove button (`src/components/mcp/McpServerForm.tsx:353-359`, used for env vars + http headers) was a bare `×` glyph with no `aria-label` — screen readers announced "button" with no hint of what would be removed. WCAG 4.1.2 (Name, Role, Value). Now carries `aria-label="Remove <KEY>"` (or "Remove row" when the key is still empty) plus a stable testid (`${prefix}-remove-${i}`). — PR #73
- [ ] _further investigation pending_
