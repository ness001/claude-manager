...
- [x] `PluginDetailView` header buttons "Open in File Browser" and "Open in VS Code" (`src/components/plugins/PluginDetailView.tsx:62, 73`) had decorative lucide icons (FolderOpen, ExternalLink) without `aria-hidden`, so screen readers announced the SVG name redundantly with the visible label text. WCAG 4.1.2 (Name, Role, Value). Mirrored PR #58 (SkillCard) and PR #55 (QuickActions): added `aria-hidden="true"` to both icons. — PR #66
...
- [x] `McpServerForm` submit-error `<p>` (`src/components/mcp/McpServerForm.tsx:260-263`) appeared after a save failure (e.g. CLI rejection) without `role="alert"`, so screen-reader users got no announcement that the save had failed — the error rendered silently. WCAG 4.1.3 (Status Messages). Mirrored PR #44 (ConversationViewer corruption banner) and PR #62 (ConversationViewer load-error banner): added `role="alert"` so the message is read without focus needing to move. — PR #72
- [ ] _further investigation pending_
