...
- [x] `ToolCallBlock` expand-toggle button contained a decorative `ChevronRight` icon next to the toolName text but the icon lacked `aria-hidden` (`src/components/conversation/ToolCallBlock.tsx:42-45`). Lucide icons render as inline SVGs, so screen readers may announce the icon's computed name redundantly with the toolName already in the sibling `<span>`. WCAG 4.1.2 Name, Role, Value. Added `aria-hidden="true"` to the chevron — minimal one-line change. — PR #53
