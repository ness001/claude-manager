...
- [x] `ActivityChart` Period (7d/30d/90d/All) and Series (Messages/Tool Calls) tab buttons (`src/components/dashboard/ActivityChart.tsx:88-104,112-141`) had no `focus-visible:ring` — keyboard users tabbing through the dashboard chart toggles had no visible focus indicator (only hover/active color swaps). Same defect class as ViewModeToggle (PR #48). WCAG 2.4.7 Focus Visible. Added `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent` to all 6 button class lists. — PR #49
