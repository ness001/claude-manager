...
- [x] `McpServerForm` arg input (`src/components/mcp/McpServerForm.tsx:214-227`) had only `placeholder="add arg + Enter"` for its accessible name — placeholders don't satisfy WCAG 4.1.2 (Name, Role, Value). Added `aria-label="Add command argument"`, mirroring the search-input fixes in PRs #45 / #50 / #51 / #60. — PR #75
- [ ] _further investigation pending_
