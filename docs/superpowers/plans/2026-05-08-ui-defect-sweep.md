...
- [x] `McpServerForm` arg-tag remove button (`src/components/mcp/McpServerForm.tsx:205-211`) was a bare `×` glyph with no `aria-label` — same defect class as the KeyValueEditor remove button (PR #73). SR users heard "button" with no hint of which arg they were about to delete. Now carries `aria-label="Remove <ARG>"` and a stable testid `form-arg-remove-<i>`. WCAG 4.1.2 (Name, Role, Value). — PR #76
- [ ] _further investigation pending_
