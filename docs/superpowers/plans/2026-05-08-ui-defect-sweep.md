...
- [x] `McpServerForm` Name input (`src/components/mcp/McpServerForm.tsx:141-148`) had no accessible name — the visual `<label>` from the `Field` wrapper isn't associated via `htmlFor`/`id`, so screen readers announce it as an unlabeled text input. Added `aria-label="Server name"` directly on the input. WCAG 4.1.2 (Name, Role, Value). — PR #77
- [ ] _further investigation pending_
