...
- [x] `SessionCard` button (`src/components/sessions/SessionCard.tsx:64-79`) had no `focus-visible:ring` — the card is the primary keyboard target in the session list, but tabbing through gave no visible focus indication (only a hover background swap). WCAG 2.4.7 Focus Visible. Added the same focus-visible class trio established by PRs #17/#45/#48/#49. — PR #56
...
- [x] `ConversationViewer` error state (`src/components/conversation/ConversationViewer.tsx:298-307`) rendered "Failed to load conversation: …" without `role="alert"`, so screen-reader users got no announcement when the JSONL load rejected (file missing, IPC error, etc.). Added `role="alert"`, mirroring the corruption-warning fix (PR #44). WCAG 4.1.3 Status Messages. — PR #62
...
- [x] `PluginListView` search input (`src/components/plugins/PluginListView.tsx:90-97`) had no accessible name — only a placeholder, which doesn't satisfy WCAG 4.1.2 (Name, Role, Value) since it disappears once the user types. The decorative magnifying-glass icon also lacked `aria-hidden`. Added `aria-label="Search plugins"` to the input and `aria-hidden="true"` to the icon, mirroring SessionSearch (PR #45), McpPanel (PR #50), SkillsListView (PR #51). — PR #60
...
- [x] `SkillsListView` skill-search input (`src/components/skills/SkillsListView.tsx:81-88`) had no accessible name — only a placeholder, which doesn't satisfy WCAG 4.1.2 (Name, Role, Value). The decorative `<Search>` icon also lacked `aria-hidden`. Same defect as McpPanel (PR #50). Added `aria-label="Search skills"` to the input and `aria-hidden="true"` to the icon, mirroring `SessionSearch` (PR #45). — PR #51
- [ ] _further investigation pending_
